...
const EventDetails = () => {
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen">
        ...
        <div className={`ml-2 text-sm font-medium ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>
  ...
  if (!event)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p className={`text-lg ${isDarkMode ? "text-gray-200" : "text-gray-500"}`}>Event not found</p>
      </div>
    );

  const containerBg = isDarkMode ? "bg-gray-900 text-white" : "bg-white text-gray-800";
  const cardBg = isDarkMode ? "bg-gray-800" : "bg-gray-50";
  const textPrimary = isDarkMode ? "text-gray-200" : "text-gray-800";
  const textSecondary = isDarkMode ? "text-gray-400" : "text-gray-500";

  return (
    <div className={`max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 my-6 sm:my-8 p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-lg transition-colors duration-500 ${containerBg}`}>
      <Toaster />
      ...
      <div className="relative rounded-xl overflow-hidden shadow-lg">
        <img src={event.thumbnail} alt={event.title} className="w-full h-48 sm:h-60 md:h-72 object-cover" />
      ...
      <div className="mt-6 sm:mt-8 text-center">
        {timeLeft.days === 0 && timeLeft.hours === 0 && timeLeft.minutes === 0 && timeLeft.seconds === 0 ? (
          ...
          <>
            <h3 className={`text-base sm:text-lg font-semibold mb-3 flex items-center justify-center gap-2 ${textPrimary}`}>
              <FaClock className="text-[#FF6B35]" /> Event starts in:
            </h3>
            <div className="grid grid-flow-col gap-2 sm:gap-3 md:gap-5 text-center auto-cols-max justify-center">
              {Object.entries(timeLeft).map(([unit, value]) => (
                <div key={unit} className={`flex flex-col p-2 rounded-lg min-w-[60px] sm:min-w-[70px] ${isDarkMode ? "bg-gray-700 text-gray-200" : "bg-gray-100 text-gray-800"}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-6">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-5">
          <p className={`leading-relaxed text-sm sm:text-base ${textSecondary}`}>{event.description}</p>

          <div className="space-y-3 sm:space-y-4">
            <div className={`flex items-center gap-2 ${textSecondary}`}>
              <FaTag className="text-[#FF6B35] flex-shrink-0" />
              <span className="font-medium">Type:</span> <span className="text-sm sm:text-base">{event.eventType}</span>
            </div>
            <div className={`flex items-center gap-2 ${textSecondary}`}>
              <FaCalendarAlt className="text-[#457B9D] flex-shrink-0" />
              <span className="font-medium">Date:</span> <span className="text-sm sm:text-base">{new Date(event.eventDate).toLocaleString()}</span>
            </div>
            <div className={`flex items-center gap-2 ${textSecondary}`}>
              <FaMapMarkerAlt className="text-[#E63946] flex-shrink-0" />
              <span className="font-medium">Location:</span> <span className="text-sm sm:text-base">{event.location}</span>
            </div>
          </div>
          ...
          <div className="mt-4 sm:mt-6">
            <h2 className={`text-lg font-semibold mb-3 flex items-center gap-2 ${textPrimary}`}>
              <FaMap className="text-[#FF6B35]" /> Event Location Map
            </h2>
            <iframe
              ...
              width="100%"
              height="300"
              className="sm:h-[350px] md:h-[400px] rounded-lg"
              style={{ border: 0, background: isDarkMode ? "#1f2937" : "#fff" }}
              allowFullScreen
              loading="lazy"
            ></iframe>
          </div>
        ...
        {/* Right */}
        <div className={`${cardBg} p-4 sm:p-5 rounded-xl shadow-md transition-colors duration-300`}>
          {/* Creator */}
          <h2 className={`text-lg font-semibold mb-4 flex items-center gap-2 ${textPrimary}`}>
            <FaUser className="text-[#FF6B35]" /> Event Creator
          </h2>
          <div className="flex items-center gap-3 sm:gap-4 mb-6">
            {event.creatorPhoto ? (
              <img src={event.creatorPhoto} alt={event.creatorName} className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border" />
            ) : (
            ...
            <div>
              ...
              <p className={`text-xs sm:text-sm ${textSecondary}`}>{event.creatorEmail}</p>
            </div>
          </div>
          ...
          <div className="mt-6 sm:mt-8">
            <h2 className={`text-lg font-semibold mb-3 flex items-center gap-2 ${textPrimary}`}>
              <FaUsers className="text-[#FF6B35]" /> Participants ({event.participants?.length || 0})
            </h2>
            {event.participants?.length > 0 ? (
              <ul className="space-y-3 max-h-56 overflow-y-auto">
                {event.participants.map((p, i) => (
                  <li key={i} className={`${isDarkMode ? "bg-gray-700" : "bg-white"} flex items-center gap-3 p-2 rounded-lg shadow-sm`}>
                    {p.photo ? (
                      <img src={p.photo} alt={p.name} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border" />
                    ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className={`text-xs sm:text-sm truncate ${textSecondary}`}>{p.email}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ) : (
              <p className={`text-sm text-center py-4 ${textSecondary}`}>No one has joined yet. Be the first!</p>
            )}
          </div>
      ...
      <div className="space-y-4 mt-6 sm:mt-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`text-base sm:text-lg font-semibold ${textPrimary}`}>Event Chat</h1>
              <p className={`text-xs sm:text-sm ${textSecondary}`}>Connect with other attendees</p>
            </div>
          </div>
...
<div className={`${cardBg} rounded-xl p-3 sm:p-4 border border-gray-200 transition-colors duration-300 
                sm:w-full w-11/12 mx-auto`}>
